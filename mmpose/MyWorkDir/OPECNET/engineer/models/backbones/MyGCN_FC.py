...
class MyGCN_FC(nn.Module):
    def __init__(self, hid_dim, coords_dim=(2, 2), p_dropout=None):
        '''
        ...
        self.adj = self._build_adj_mx_from_edges(21, get_adj())

        super(MyGCN_FC, self).__init__()
        ...
        self.heat_map_generator = HM_Extrect(21)

        self.heat_map_head.append(self.heat_map_generator)

        self.gconv_input = _GraphConv(self.adj, coords_dim[0], hid_dim[0], p_dropout=p_dropout)
        # in here we set 4 gcn model in this part
        self.gconv_layers1 = _ResGraphConv_Attention(self.adj, hid_dim[0], hid_dim[1], hid_dim[0], p_dropout=p_dropout)
        self.gconv_layers2 = _ResGraphConv_Attention(self.adj, hid_dim[1] + 256, hid_dim[2] + 256, hid_dim[1] + 256,
                                                     p_dropout=p_dropout)
        self.gconv_layers3 = _ResGraphConv_Attention(self.adj, hid_dim[2] + 384, hid_dim[3] + 384, hid_dim[2] + 384,
                                                     p_dropout=p_dropout)
        self.gconv_layers4 = _ResGraphConv_Attention(self.adj, hid_dim[3] + 512, hid_dim[4] + 512, hid_dim[3] + 512,
                                                     p_dropout=p_dropout)

        self.gconv_output1 = SemGraphConv(384, coords_dim[1], self.adj)
        self.gconv_output2 = SemGraphConv(512, coords_dim[1], self.adj)
        self.gconv_output3 = SemGraphConv(640, coords_dim[1], self.adj)
    ...
    def forward(self, x_gcn, heat_map, features):
        """

        Args:
            x_gcn: torch(b, 21, 4) 代表(pred_x, pred_y, score, t)
            heat_map: torch(b, 21, 64, 64)
            features: list. 里面一共有两个heatmaps.
        ...
        bs = heat_map.shape[0]
        heat_map_intergral = self.FC(heat_map.view(bs * 21, -1)).view(bs, 42)
        hm_4 = heat_map_intergral.view(-1, 21, 2)
        # grid_sample是torch.nn.functional包下的函数。根据这个函数得到三个特征
        j_1_16 = F.grid_sample(features[0], hm_4[:, None, :, :]).squeeze(2)  #
        j_1_8 = F.grid_sample(features[1], hm_4[:, None, :, :]).squeeze(2)  #
        j_1_4 = F.grid_sample(features[2], hm_4[:, None, :, :]).squeeze(2)  #

        # x = torch.cat([hm_4,score],-1)
        out = self.gconv_input(x_gcn)
        ...
        return [out1, out2, out3], heat_map_intergral, x_gcn
