...
class _ResGraphConv(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.gconv2(out)
        out = self.bn(residual.transpose(1, 2).contiguous() + out)
        out = self.relu(out).transpose(1, 2).contiguous()
        return out
...
class Node_Attention(nn.Module):
    def __init__(self, channels):
        ...
        #
        # )
        self.Liner1 = nn.Linear(channels, channels // 4)
        self.BN1 = nn.BatchNorm1d(channels // 4)
        self.RELU = nn.ReLU()
        self.Liner2 = nn.Linear(channels // 4, 21)
    ...
    def forward(self, x):
        ...
        out = self.Liner1(out)  # TODO 每次都是在这一步出现nan. 出现nan是因为weight出现nan
