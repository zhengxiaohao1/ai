...
def generate_2d_integral_preds_tensor(heatmaps, num_joints, x_dim, y_dim):
    ...
    accu_x = accu_x * torch.arange(x_dim).float().cuda()[None, :]  # (torch.arange(x_dim).type(torch.cuda.FloatTensor), devices=[accu_x.device.index])[0]
    accu_y = accu_y * torch.arange(y_dim).float().cuda()[None, :]  # (torch.arange(y_dim).type(torch.cuda.FloatTensor), devices=[accu_y.device.index])[0]
