...
def transformBox_batch(pt_, ul, br, inpH, inpW, resH, resW):
    #inverse method by gt_points to resH,resW area

    # in pytorch 1.1, there are bugs. so i use pt = pt_.copy() replace original pt
    pt = pt_.copy()

    num_joints = pt.shape[1]
    center = (br - 1 - ul) / 2
    size = br - ul
    ...
    #cordination_transfer
    _pt[:, :, 0] = _pt[:, :, 0] + ((lenW[:, np.newaxis].repeat(1, num_joints) - 1) /
                                   2 - center[:, 0].unsqueeze(-1).repeat(1, num_joints)).clamp(min=0)
    _pt[:, :, 1] = _pt[:, :, 1] + ((lenH[:, np.newaxis].repeat(1, num_joints) - 1) /
                                   2 - center[:, 1].unsqueeze(-1).repeat(1, num_joints)).clamp(min=0)


    _pt = _pt*(resH/lenH[:, np.newaxis, np.newaxis])

    _pt[:,:,0] = _pt[:,:,0].clamp(min=0,max=resW-1)
    _pt[:,:,1] = _pt[:,:,1].clamp(min=0,max=resH-1)
