...
from MyWorkDir.OPECNET.engineer.SPPE.src.utils.img import flip, shuffleLR
from MyWorkDir.OPECNET.engineer.SPPE.src.models.FastPose import createModel
...
class InferenNet_fast(nn.Module):
    def __init__(self, kernel_size, dataset):
        ...
        model = createModel().cuda()
        print('Loading pose model from {}'.format('./weights/sppe/duc_se.pth'))
        model.load_state_dict(torch.load('./weights/sppe/duc_se.pth'))
        model.eval()
        self.pyranet = model
    ...
    def forward(self, x):
        out, ret_features = self.pyranet(x)
        out = out.narrow(1, 0, 17)
